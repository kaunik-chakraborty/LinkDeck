...
object GuideRepository {
    ...
    fun getSimpleGuideItems(): List<GuideItem> = listOf(
        ...
        GuideItem(
            id = "de_amping_engine",
            index = 3,
            title = "Automatic On-Device De-AMPing",
            description = "Unwraps Google AMP viewers, AMP Project CDN caches, Cloudflare/Bing AMP caches, and publisher AMP subdomains into direct canonical publisher URLs with zero network calls and zero tracking.",
            example = "https://www.google.com/amp/s/reddit.com/r/android\n→ Unwrapped to: https://reddit.com/r/android\n(Immediately unlocks Android native app discovery so you can open the Reddit app directly instead of being trapped in Google Search!)"
        ),
        GuideItem(
    ...
    fun getTechnicalGuideItems(): List<GuideItem> = listOf(
        ...
        GuideItem(
            id = "deamp_engine_tech",
            index = 3,
            title = "RFC 3986 On-Device De-AMPing Engine",
            description = "• Pure on-device URI and regex parser unrolls Google AMP viewers (/amp/s/), Google search click redirects (/url?q=), AMP Project CDN caches (*.cdn.ampproject.org), Cloudflare/Bing caches, and publisher subdomains (amp.example.com).\n• Reconstructs canonical publisher URLs with UTF-8 decoding, strips AMP tracking parameters (amp_js_v, amp_gsa, usqp), and preserves functional parameters and fragment anchors.\n• Completely offline with zero network lookups, zero telemetry, and full SSRF safety validation."
        ),
        GuideItem(
            id = "tls_inspector",
            index = 4,
